#ifndef WDONG_KGRAPH_DATA_ARM
#define WDONG_KGRAPH_DATA_ARM
...
// Define a standard alignment value
static constexpr size_t KGRAPH_MATRIX_ALIGN = 64;
...
protected:
    void reset (unsigned r, unsigned c) {
        ...
        if (data) free(data);
        data = (char *)memalign(A, row * stride); // Aligned allocation
...
public:
    ...
    MatrixProxy (flann::Matrix<DATA_TYPE> const &m)
        ...
        if (stride % A) throw std::invalid_argument("bad alignment");
    }
#endif
